...
let presetAtlanticDolphin = ['AtlanticDolphin', 24000, 0, -50, -60, 10, 22050];
let presetMinke 					= ['Minke', 6000, 0, -35, -60, 10, 22050];
let presetHumpback 				= ['Humpback', 6000, 0, -35, -60, 10, 22050];
let presetBeakedDolphin 	= ['BeakedDolphin', 24000, 0, -60, -70, 10, 22050];
let presetBeardedSeal 		= ['BeardedSeal', 6000, 0, -35, -60, 10, 22050];
let presetHarborPorpoise 	= ['HarborPorpoise', 6000, 0, -35, -60, 10, 22050];
...
function setup() {
  ...
  // Create our bins of frequencies
	...
	// ----- FMAX -----
	// Slider
	fmaxSlider = createSlider(0, nyquist, fmaxDefault).hide();
	...
	// ----- FMIN -----
	// Slider
	fminSlider = createSlider(0, nyquist, fminDefault).hide();
...
function draw() {
	...
	for ( i = 0; i< spectrum.length; i++){
		...
		if (wholeSoundModeFlag == false) {
			let eraserRadius = map(i, 0, numberOfBins, 1, circleScale, true); // Hz from 1 to 23kHz -> 10 to 100 radius
