...
function setup() {
  ...
  // Create our bins of frequencies
	...
	// ----- FMAX -----
	// Slider
	fmaxSlider = createSlider(0, nyquist, 5000).hide();
	...
	// ----- FMIN -----
	// Slider
	fminSlider = createSlider(10, nyquist, 0).hide();
	...
	// ----- DBMAX -----
	// Slider
	dbmaxSlider = createSlider(-90, -20, -27).hide();
	...
	// ----- DBMIN -----
	// Slider
	dbminSlider = createSlider(-90, -20, -55).hide();
...
function draw() {
	...
	if (wholeSoundModeFlag == false) {
	  	...
	  	let eraserRadius = (map(i, 0, numberOfBins*1, 1, circleScale, true)); // Hz from 1 to 23kHz -> 10 to 100 radius
	  ...
	  //  ---------- SOUND LINES ----------
    	...
    	if (spectrum[i] > dbminSlider.value()) {
			...
			if (peakFrequency => fminSlider.value() && peakFrequency <= fmaxSlider.value()) {
				...
				let thickness = map(spectrum[i], dbminSlider.value(), dbmaxSlider.value(), 0.1, 2.1, true);
